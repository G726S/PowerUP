...
export function HomePage() {
  ...
  async function handleCreate(name: string, files: File[]) {
    ...
    try {
      await api.uploadMaterials(course.id, files);
    } finally {
      await refresh();
    }
    ...
    <div className="mx-auto max-w-6xl px-6 py-10 md:px-10 md:py-14">
        ...
        <p className="font-mono text-sm text-[var(--color-muted)]">
          {courses === null
            ...
            : courses.length === 0
              ...
              : `${courses.length} ${courses.length === 1 ? "COURSE" : "COURSES"} · ${totalMaterials} ${totalMaterials === 1 ? "MATERIAL" : "MATERIALS"}`}
        </p>
      </header>
      ...
      {courses === null ? (
        ...
        <>
          ...
          {filtered && filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {filtered?.map((course) => (
                <CourseCard key={course.id} course={course} onRequestDelete={setPendingDelete} />
              ))}
            </div>
          )}
        </>
      ...
      {pendingDelete && (
        <ConfirmDialog
          ...
          description="This removes the course and all of its materials. This can't be undone."
